# 原文地址：https://blog.csdn.net/weixin_43594279/article/details/129779132
...
class PositionalEmbedding(nn.Module):
    def __init__(self, max_seq_len, embed_model_dim):
        ...
        pe = torch.zeros(max_seq_len, self.embed_dim) # 零张量，存储位置嵌入
        for pos in range(max_seq_len):
            for i in range(0, self.embed_dim, 2):
                pe[pos, i] = math.sin(pos / (10000 ** ((2 * i) / self.embed_dim)))
                pe[pos, i + 1] = math.cos(pos / (10000 ** ((2 * (i + 1)) / self.embed_dim)))
        # 增加一个batch_size维度，为了能够与transformer的输入相加，即将其变为一个张量
        #  tensor([[ 0.0000,  1.0000,  0.0000,  1.0000],
        #         [ 0.8415,  0.5403,  0.0001,  0.9999],
        #         [ 0.9093, -0.4161,  0.0002,  0.9998],
        #         [ 0.1411, -0.9900,  0.0003,  0.9997],
        #         [-0.7568, -0.6536,  0.0004,  0.9996]])
        #
        #  tensor([[[ 0.0000,  1.0000,  0.0000,  1.0000],
        #          [ 0.8415,  0.5403,  0.0001,  0.9999],
        #          [ 0.9093, -0.4161,  0.0002,  0.9998],
        ...
        #          [-0.7568, -0.6536,  0.0004,  0.9996]]])
        pe = pe.unsqueeze(0)
        # 将pe注册为buffer，这样在模型被拷贝或者重载时，pe也会被拷贝或者重载；且不会被优化器优化
        self.register_buffer('pe', pe)

    def forward(self, x):
        # 在许多Transformer模型实现中，放大嵌入向量有助于稳定训练过程，提高模型的表现
        x = x * math.sqrt(self.embed_dim)
        # 获取输入张量x的序列长度，即size(1)
        seq_len = x.size(1)
        # 张量相加
